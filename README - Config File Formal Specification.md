...
## Validation rules
- `StashConfig.Defaults.URLRoot`: required **only when the config contains at least one HTTP request that does not define its own request-level `URLRoot`** (may be omitted for AMQP-only configs, or when every HTTP request carries its own `URLRoot`). Accepts a non-empty string or a `$secrets`/`$dynamic`/`$pattern`/`$timestamp`/`$func: timestamp` operator.
...
- `URLRoot`: string OR `$secrets`/`$dynamic`/`$pattern`/`$timestamp`/`$func: timestamp` operator (optional; required only when the config contains an HTTP request without its own request-level `URLRoot`; resolved like Headers/Body/Query)
...
- `URLPath`: string OR `$secrets`/`$dynamic`/`$pattern`/`$timestamp`/`$func: timestamp` operator (required; resolved like Headers/Body/Query, incl. `${captured:KEY}` at request time)
- `URLRoot`: string OR `$secrets`/`$dynamic`/`$pattern`/`$timestamp`/`$func: timestamp` operator (optional; overrides `Defaults.URLRoot` for this request only; explicit null is invalid, while omission inherits the default; same resolution rules)
- `Headers`: map<string, any> (optional)
- `Body`: map<string, any> (optional)
- `Query`: map<string, any> (optional)
...
- Shorthand form:
  ...
  ```
  where `format` is one of: `iso_8601` (default), `epoch_ms`, or `epoch_s`.
